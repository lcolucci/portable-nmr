function varargout=sigstar2(model_series, groups,stats,nosort,plotType, figAxes)
    % sigstar - Add significance stars to bar charts, boxplots, line charts, etc,
    %
    % --- Lina's notes to self for using this function (including my modifications) ----
    % model_series = max height of each boxplot/bar
    %             [group1_a group1_b
    %              group2_a group2_b
    %              group3_a group3_b
    %              group4_a group4_b]
    % groups = cell column of group names, i.e. {'Pre'; 'Post'}
    % stats = array of the same length as group names with the pValue, i.e.[0.03, 0.001]
    % nosort = 0 (I don't think I've been able to make '1' work)
    % plotType = 'bar' or 'boxplot' (depending on plot type, finds 'xlocs' differently)
    % figAxes = the handle of the figure axes that drawing on top of
    % -----------------------------------------------------------------------------------
    %
...
%     end
    ...
    %% *********** This section changed from original sigstar function to work w/ grouped bar graphs *************
    %  One assumption in both 'bar' and 'boxplot' is that sigstar lines are
    %  just drawn btw neighboring bars/boxplots. I should add a 'manual'
    %  option at some point where custom options can be drawn. 
    
    if strmatch(plotType,'bar')
    ...
    if strmatch(plotType,'boxplot')
        % modelSeries, max height of each boxplot/bar
        %             [group1_a group1_b
        %              gruop2_a group2_b
        %              group3_a group3_b]
        ngroups = size(model_series,1); 
        
        lines = figAxes.Children; % get handles to the lines
        uw = findobj(lines, 'tag', 'Upper Whisker');  % get handle to "Upper Whisker" line
        nBoxplots = size(uw,1); 
        positions = nan(nBoxplots,1); 
        for i=1:nBoxplots
            positions(i) = uw(i).XData(1); 
        end
        positions = sort(positions); 
        positionMatrix = nan(2,length(positions)./2); 
        for i=1:length(positions)
            positionMatrix(i) = positions(i);
        end
        xlocs = positionMatrix'; 
    end
...
function H=makeSignificanceBar(x,y,p)
    ...
    if ~isnan(p) && p<=0.05
        ...
        offset=0.02; %0.02 when default font size
    end
