function [ogroup,glabel,gname,multigroup,maxgroup] = mgrp2idx_lina(group,rows,sep)
% Direct copy of Matlab's built-in function so that it's accessible
% 
...
if (~multigroup)
    ...
    if iscell(group)
        ...
        for j=1:ngrps
            [g,gn] = statslib.internal.grp2idx(group{1,j});
            if j==1
                rows = size(g,1);
        ...
        grpmat = zeros(rows,ngrps);
        for j=1:ngrps
            [g,gn] = statslib.internal.grp2idx(group(:,j));
            grpmat(:,j) = g;
            namemat{1,j} = gn;
        end
    end
